  ...
  const startDecoding = (stream: MediaStream) => {
    ...
    reader.current.decodeFromStream(stream, video.current!, (result, err) => {
      if (result && canvas.current) {
        ...
        const field = String(state?._field ?? 'qrcode')
        context?.setState?.({ ...state, _value: { [field]: qrcode } })
        setTimeout(() => {
          stopCamera()
          navigate(-1)
        }, 50)
      ...
      {state === CameraStates.ALLOWED && (
        <>
          ...
          {!image && <Tile
            ...
            width={width}
            height={height}
            cropWidth={{ value: '90%', maxWidth: 300, minWidth: 250 }}
            cropHeight={{ value: 300, minHeight: 250 }}
          />}
          {image ? (
